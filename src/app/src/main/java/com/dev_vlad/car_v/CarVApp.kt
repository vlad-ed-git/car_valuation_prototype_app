...
class CarVApp : Application() {
    ...
    val carRepo by lazy { CarRepo(database.carEntityDao(), Glide.with(applicationContext)) }
    val offerRepo by lazy { OffersRepo(database.carOfferEntityDao()) }
    val chatRepo by lazy { ChatRepo(database.chatDao(), Glide.with(applicationContext)) }
}
